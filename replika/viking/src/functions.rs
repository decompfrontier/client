...
impl Info {
    ...
    pub fn get_start(&self) -> u64 {
        self.addr | repo::get_config().base_address
    }
...
pub fn parse_address(value: &str) -> Result<u64> {
    Ok(parse_base_16(value)? - repo::get_config().base_address)
}
...
pub fn get_functions_csv_path(version: Option<&str>) -> PathBuf {
	...
	let final_csv = repo::get_data_path(version).expect("Failed to get data path").join(functions_csv);
    path.push(final_csv);
